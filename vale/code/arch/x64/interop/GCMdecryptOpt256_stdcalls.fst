...
open Vale.Stdcalls.GCMdecryptOpt 
  ...
  Stack UInt64.t
    ...
    (ensures fun h0 c h1 ->
      ...
      (8 * (UInt64.v cipher_num) < pow2_32 /\
       ...
       in let cipher_bytes = slice_work_around (le_seq_quad32_to_bytes cipher_in) (UInt64.v cipher_num)
       in let plain_out =
           Seq.append (Seq.append (UV.as_seq h1 out128x6_u) (UV.as_seq h1 out128_u))
                      (UV.as_seq h1 inout_u)
       in let plain_bytes = slice_work_around (le_seq_quad32_to_bytes plain_out) (UInt64.v cipher_num)
      ...
      4096 * (Seq.length cipher_bytes) < pow2_32 /\
      4096 * (Seq.length auth_bytes) < pow2_32 /\
      is_aes_key AES_256 (seq_nat32_to_seq_nat8_LE (Ghost.reveal key)) /\
      (let plain, result = gcm_decrypt_LE AES_256 (seq_nat32_to_seq_nat8_LE (Ghost.reveal key)) (be_quad32_to_bytes iv_BE) cipher_bytes auth_bytes expected_tag in
...
inline_for_extraction
let gcm256_decrypt_opt' key auth_b auth_bytes auth_num keys_b iv_b hkeys_b abytes_b
  ...
  let x, _ = gcm256_decrypt_opt  key auth_b auth_bytes auth_num keys_b iv_b hkeys_b abytes_b
  ...
  Stack UInt64.t
    ...
    (ensures fun h0 c h1 ->
      ...
      (let cipher_d = get_downview cipher_b in
       ...
       let cipher_bytes = slice_work_around (le_seq_quad32_to_bytes cipher_in) (UInt64.v cipher_len) in
       let plain_out = Seq.append (UV.as_seq h1 out_u) (UV.as_seq h1 inout_u) in
       let plain_bytes = slice_work_around (le_seq_quad32_to_bytes plain_out) (UInt64.v cipher_len) in
      ...
      4096 * (Seq.length cipher_bytes) < pow2_32 /\
      4096 * (Seq.length auth_bytes) < pow2_32 /\
      (let plain, result = gcm_decrypt_LE AES_256 (seq_nat32_to_seq_nat8_LE (Ghost.reveal key)) iv cipher_bytes auth_bytes expected_tag in
      Seq.equal plain plain_bytes /\
      (UInt64.v c = 0) == result
      )
    ))
...
inline_for_extraction
let gcm256_decrypt_opt_alloca key cipher_b cipher_len auth_b auth_bytes iv_b
...
let lemma_identical_uv (b:uint8_p) (h0 h1:HS.mem) : Lemma
  ...
  = lemma_dv_equal Views.down_view8 b h0 h1;
    length_aux3 b (B.length b / 16);
    lemma_uv_equal Views.up_view128 (get_downview b) h0 h1
...
let lemma_slice_uv_extra (b:uint8_p) (b_start:uint8_p) (b_extra:uint8_p) (h:HS.mem) : Lemma
  ...
  (ensures (
    ...
    length_aux6 b_start;
    let b_start_u = UV.mk_buffer b_start_d Views.up_view128 in
    let b_extra_d = get_downview b_extra in
    length_aux6 b_extra;
    let b_extra_u = UV.mk_buffer b_extra_d Views.up_view128 in
...
inline_for_extraction
let gcm256_decrypt_opt_stdcall key cipher_b cipher_len auth_b auth_len iv_b out_b tag_b keys_b hkeys_b scratch_b =
  let h0 = get() in
  ...
  let scratch_b = B.sub scratch_b 32ul 128ul in
  ...
  assert (
    ...
    let plain, result = gcm_decrypt_LE AES_256 (seq_nat32_to_seq_nat8_LE (Ghost.reveal key)) iv cipher auth expected_tag in
